...
public class CheciController {
    ...
    @GetMapping("/panduan/{dengji}/{id}")
    public R panduan(@PathVariable String dengji,@PathVariable Integer id) throws NoSuchFieldException, IllegalAccessException {
        CheciEntity checiEntity = checiService.selectById(id);

        // 获取类的 Class 对象
        Class<?> clazz = checiEntity.getClass();
        // 获取 checiValue 属性
        Field checiValueField = clazz.getDeclaredField(dengji);
        // 设置可访问私有属性
        checiValueField.setAccessible(true);
        // 获取属性值
        Float dengjiva = (Float) checiValueField.get(checiEntity);
        if(ObjectUtils.isEmpty(dengjiva)){
            return R.error("该座位无票");
        }
        if(dengjiva==0){
            return R.error("该座位无票");
        }
        return R.ok().put("data",dengjiva);
    }


    @GetMapping("/zhifu")
    @IgnoreAuth
    public void zhifu(HttpServletResponse httpResponse) throws IOException, AlipayApiException {
        alipayTemplate.pay(httpResponse);
    }
...
//            return R.error(511,"永远不会进入");
        ...
        if(ObjectUtils.isEmpty(checi.getYideng())){
            checi.setYideng((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getErdeng())){
            checi.setErdeng((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getShangwu())){
            checi.setShangwu((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getDongwo())){
            checi.setDongwo((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getYingzuo())){
            checi.setYingzuo((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getYingwo())){
            checi.setYingwo((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getRuanzuo())){
            checi.setRuanzuo((float) 0);
        }
        if(ObjectUtils.isEmpty(checi.getRuanwo())){
            checi.setRuanwo((float) 0);
        }
