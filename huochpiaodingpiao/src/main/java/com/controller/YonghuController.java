...
public class YonghuController {
    ...
    static class RequestBodya {
        String model;
        Message[] messages;

        public RequestBodya(String model, Message[] messages) {
            this.model = model;
    ...
    @PostMapping("/wenda")
    @IgnoreAuth
    public R wenda(@RequestBody Zaiti zaiti) throws Exception {
        String neirong = zaiti.getNeirong();
        // 创建请求体
        RequestBodya requestBody = new RequestBodya(
                //此处以qwen-plus为例，可按需更换模型名称。模型列表：https://help.aliyun.com/zh/model-studio/getting-started/models
                "qwen-max",
                new Message[] {
                        new Message("system", "你是一个交通的专家，帮助提问者回答问题."),
                        new Message("user", neirong)
                }
        );
        // 将请求体转换为 JSON
        Gson gson = new Gson();
        ...
        String auth = "Bearer sk-a6c2c1d6514f44a1a56781f45cc8af33" ;
...
//                System.out.println("Response Body: " + response);
            ...
            root = JSON.parseObject(response.toString(), Root.class);
            System.out.println(root.getChoices().get(0).getMessage().getContent());
        }
        return R.ok(root.getChoices().get(0).getMessage().getContent());
    }
    ...
    public R bangding(@RequestBody YonghuEntity yonghu, HttpServletRequest request){
        if(yonghu==null){
            return R.error(511,"不可为空");
        }else {
    ...
    @PostMapping("/zhaohui")
    @IgnoreAuth
    public R zhaohui(@RequestBody YonghuEntity yonghu){
        if(yonghu==null){
            return R.error(511,"不可为空");
        }else {
            EntityWrapper<YonghuEntity> wrapper = new EntityWrapper<>();
            wrapper.eq("username",yonghu.getUsername());
            wrapper.eq("yonghu_name",yonghu.getYonghuName());
            wrapper.eq("yonghu_phone",yonghu.getYonghuPhone());
            wrapper.eq("yonghu_id_number",yonghu.getYonghuIdNumber());
            List<YonghuEntity> yonghuEntities = yonghuService.selectList(wrapper);
            if(!ObjectUtils.isEmpty(yonghuEntities)){
                return R.ok().put("data","你的密码是："+yonghuEntities.get(0).getPassword()+"，请妥善保存");
            }
            return R.ok().put("data","找不到该用户");
        }
    }
    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        for (YonghuView yonghuView : list) {
            Integer zhekou = yonghuView.getZhekou();

            if(!ObjectUtils.isEmpty(zhekou)){
                if(zhekou!=0) {
                    Zhekou zhekou1 = zhekouService.selectById(zhekou);
                    yonghuView.setZhekouValue(zhekou1.getZhekou());
                }
            }
